import Foundation

@MainActor
final class CreateWalletViewModel: ObservableObject {

    @Published var label: String = ""
    @Published var network: BitcoinNetwork = .mainnet
    @Published var isCreating = false
    @Published var errorMessage: String?
    @Published var createdWallet: WalletInfo?

    var walletClient: WalletClient?

    var canCreate: Bool {
        !label.trimmingCharacters(in: .whitespaces).isEmpty && !isCreating
    }

    func createWallet() async {
        guard canCreate, let client = walletClient else { return }

        isCreating = true
        errorMessage = nil

        do {
            createdWallet = try await client.createWallet(label: label.trimmingCharacters(in: .whitespaces), network: network)
        } catch {
            errorMessage = error.localizedDescription
        }

        isCreating = false
    }
}
